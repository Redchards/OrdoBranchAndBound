...
for nb_t in range(3, 25):
    ...
    for i in range(0, a):
        inpt = generate_non_correlated_instance(nb_t)
        beg = time.time()
        tree.eval_greed(nb_t, inpt)
        end = time.time()
        res_eval_b1[nb_t - 3] += end - beg
        
        beg = time.time()
        tree.eval_LDS(nb_t, inpt, eval_inf_b1, 1)
        end = time.time()
        res_eval_b2[nb_t - 3] += end - beg
        
        beg = time.time()
        tree.eval_LDS(nb_t, inpt, eval_inf_b1, 2)
        end = time.time()
        res_eval_b3[nb_t - 3] += end - beg
        
        beg = time.time()
        if i <= 16:
            tree.eval_LDS(nb_t, inpt, eval_inf_b1, 4)
        end = time.time()
        if nb_t <= 16:
...
ax.set_ylabel("temp d'exécution moyen (en secondes)")
x = list(range(3, 25))
ax.legend(handles=[mpatches.Patch(color='red', label="Branch&Greed"),
                    mpatches.Patch(color="yellow", label="LDS, k=1"),
                    mpatches.Patch(color="green", label="LDS, k=2"),
                    mpatches.Patch(color='blue', label="LDS, k=4"),
                    mpatches.Patch(color='purple', label="Johnson")])
ax.plot(x, res_eval_b1, "r")
ax.plot(x, res_eval_b2, "y")
ax.plot(x, res_eval_b3, "g")
ax.plot(list(range(3, 17)), res_eval_max, "b")
ax.plot(x, res_eval_j, "purple")
...
#
#beg = time.time()
#res1, res2 = tree.eval2(nb_t, inpt, eval_inf, nb_t)
