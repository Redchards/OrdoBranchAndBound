...
for nb_t in range(3, 11):
    ...
    for i in range(0, a):
        ...
        beg = time.time()
        tree.eval2(nb_t, inpt, eval_inf_max, 1)
        end = time.time()
        res_eval_b2[nb_t - 3] += end - beg
        
        beg = time.time()
        tree.eval2(nb_t, inpt, eval_inf_max, 2)
        end = time.time()
        res_eval_b3[nb_t - 3] += end - beg
        
        beg = time.time()
        tree.eval2(nb_t, inpt, eval_inf_max, nb_t)
        end = time.time()
...
x = list(range(3, 11))
ax.legend(handles=[mpatches.Patch(color='red', label="eval"),
                    mpatches.Patch(color="blue", label="eval2, k=1"),
                    mpatches.Patch(color="yellow", label="eval2, k=2"),
                    mpatches.Patch(color='green', label="eval2, k=n")])
ax.plot(list(range(3, 11)), res_eval_b1, "r")
